"""
Record and replay test for API network error handling
"""
import sys
import unittest
...
class TestWeatherApp9(unittest.TestCase):
    def setUp(self):
        ...
        self.recording_file = 'test_recordings/network_error.json'
    ...
    def test_network_error(self):
        """Tests behavior when network error occurs during API call"""
        if self.record_mode:
            # In record mode, we need to simulate a network error
            # This would normally come from the real API

            # For recording purposes, we'll mock a network error
            self.mock_weather_api.side_effect = ConnectionError("Network connection error")
            ...
            # Mock the weather API to simulate network error
            self.mock_weather_api.side_effect = ConnectionError("Network connection error")
